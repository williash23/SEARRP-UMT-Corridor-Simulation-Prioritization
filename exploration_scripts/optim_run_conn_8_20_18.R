...
# =============================================================================
	...
	moves_in_tmp <- raster("C:/Users/saraw/Documents/SEARRP_Analyses/conn_move_sim/moves_in/1000sim_3000steps_init_loc_wt_pa_size_over5000h_turn_conc80_lg_area/moves_in.grd")
	...
	conn_in_tmp <- overlay(moves_in, pt_wt_r_01, fun=function(x, y, na.rm = TRUE) {return(x + y)})
	x_min <- conn_in_tmp@data@min
	x_max <- conn_in_tmp@data@max
	conn_in <- raster::calc(conn_in_tmp, range01)
	
	conn_in <- stack(moves_in, pt_wt_r)
...
# =============================================================================
		...
		p_init <- problem(x = pu_in, features = moves_in, cost_column = "const_cost") %>%
			...
			add_relative_targets(0.5/cellStats(moves_in, stat='sum')) %>%
		...
		if(any(diff_pa_tst > 0) == FALSE){
			...
			for(i in 2:niter_max_corr){
				...
				p <- problem(x = upd_pu_tmp, features = upd_feat_tmp, cost_column = "const_cost") %>%
					add_max_cover_objective(1200) %>%
					#add_min_set_objective() %>%
					#add_relative_targets(1/cellStats(moves_in, stat='sum')) %>%
