...
# =============================================================================
	...
	# # ----------------------
	# #  Remove planning units near Maliau-Danum complex.
	# pu_md_int <- st_erase(pu_sf, mal_dan_bound) %>%
		# mutate(size = as.integer(st_area(.)*0.0001)) %>%
		# dplyr::filter(size > 450)
	# thresh_d <- set_units(10, m)
	# pu_clust1 <- clusterSF(pu_md_int, thresh_d)
	# pu_clust1_filter <- pu_clust1 %>%
		# mutate(size = as.integer(st_area(.)*0.0001)) %>%
		# dplyr::filter(size > 10000)
	# thresh_d <- set_units(5000, m)
	# pu_clust2 <- clusterSF(pu_size, thresh_d)
	# pu_clust2_filter <- pu_clust2 %>%
		# mutate(size = as.integer(st_area(.)*0.0001)) %>%
		# dplyr::filter(size > 100000)
	# pu_bound <- st_union(pu_clust2_filter, by_feature = TRUE)
	# pu_bound_int <- st_intersection(pu_sf, pu_bound)
	# pu_in_sf <- pu_bound_int %>%
		# mutate(size = as.integer(st_area(.)*0.0001)) %>%
		# dplyr::filter(size > 490)
	# save(pu_in_sf, file = "C:/Users/saraw/Documents/SEARRP_Analyses/optimization/pu_in_sf.Rdata")
...
# =============================================================================
		...
		# # ----------------------
		# #  Set up initial problem planning units (only those near existing - PAs)
		# pu_pa_b <- st_buffer(pa_in_b_lg, 1000)
		# pu_pa_int <- st_intersection(pu_sf, pu_pa_b) %>%
			# mutate(size = as.integer(st_area(.)*0.0001)) %>%
			# dplyr::filter(size > 450)
		# pu_in_init <- as(pu_pa_int, "Spatial")
		
		# pu_pa_inter <- st_intersects(pu_sf, pa_in_b_lg, sparse = FALSE)
		# pu_pa_inter[pu_pa_inter == TRUE] <- 1
		# pu_in_init_num <- as.data.frame(pu_pa_inter) %>%
			# mutate(inters = rowSums(.)) %>%
			# mutate(pu_num = 1:n()) %>%
			# dplyr::filter(inters > 0) %>%
			# dplyr::select(pu_num)
		# pu_sf_num <- pu_sf %>%
			# mutate(pu_num = 1:n())
		# pu_in_init_sf <- st_sf(left_join(pu_in_init_num, pu_sf_num, by = "pu_num"))
		# pu_in_init <- as(pu_in_init_sf, "Spatial")
