...
# =============================================================================
	...
	#corr_sim_m <- raster("corr_sim_m.tif")
	max_width <- 25
...
# =============================================================================
		...
		p_init <- problem(x = pu_p, features = corr_sim_m, cost_column = "mainland_sabah_planning_units") %>%
			...
			#add_max_utility_objective(200005) %>%
		...
		width_tst <- as.numeric(max(st_distance(st_cast(st_geometry(s_1_sf),"POINT"))))/1000
		width_tst > 25
		...
		for(i in 2:50){
			...
			p <-  problem(x = pu_p, features = cnew_corr, cost_column = "mainland_sabah_planning_units") %>%
			...
			# ----------------------
			#  Set up locked in areas (previous solution) for next round of selection.
			tmp 4 <- st_as_sf(tmp3) %>% dplyr::filter(solution_1 == 1) %>% dplyr::select(solution_1)
			...
			width_tst <- as.numeric(max(st_distance(st_cast(st_geometry(tmp5),"POINT"))))/1000
			width_tst
			#if(width_tst > 25) {break}
		
			}
			
# stopped at i = 4			
			
			# For ifelse to remove boundary penalty
				# ----------------------
				#  Specify that planning unit selected in initiation or previous iteration is locked in.
				...
				p <- problem(x = pu_r, features = corr_sim_m) %>%
					add_max_utility_objective(budget = locked_in_area + 10001) %>%
					add_locked_in_constraints(locked_in) %>%
					#add_contiguity_constraints() %>%
					#add_max_cover_objective(budget = locked_in_area + 50001) %>%
					#add_relative_targets(0.1) %>%
					#add_relative_targets(feat_cov + 0.1) %>%
				...
				feat_cov <- as.numeric(feature_representation(p, tmp3)[3])
				feat_cov
				cellStats(locked_in, sum)
				cellStats(tmp3, sum)
				plot(pu_r, col = "dodgerblue")
				plot(tmp3, add = T, col = c("transparent", "red"))
				plot(locked_in, add = T, col = c("transparent", "pink"))
				plot(pa_sp, add = T, col = "black")
		...
		#for(j in 2:20){
			...
			# ----------------------
			#  Initial problem
			p_init_loop <- problem(x = pu_start, features = corr_sim_m) %>%
				add_max_utility_objective(5005) %>%
				add_locked_out_constraints(locked_out) %>%
				#add_max_utility_objective(2505) %>%
				add_contiguity_constraints() %>%
				add_boundary_penalties(penalty = 0.1) %>%
				add_binary_decisions() %>%
				add_gurobi_solver(time_limit = 600)
			...
			#for(i in 2:25){
				...
				nam2 <- paste("s", i-1, "sf_loop", sep = "_")
				tmp <- get(nam2)
				new_corr <- raster::mask(corr_sim_m, st_buffer(tmp, 10000))
		
				# ----------------------
				#  Set up problem to add planning units to first planning unit selected.
				p_loop <- problem(x = pu_r, features = new_corr) %>%
					add_max_utility_objective(budget = locked_in_area + 5001) %>%
					#add_max_utility_objective(budget = locked_in_area + 2505) %>%
					add_locked_in_constraints(locked_in) %>%
					add_locked_out_constraints(locked_out) %>%
					add_contiguity_constraints() %>%
				...
				if(corr_overlap > 100000) {break}
					...
					feat_cov <- as.numeric(feature_representation(p_loop, tmp3)[3])
					feat_cov
					cellStats(locked_in, sum)
					cellStats(tmp3, sum)
					#plot(pu_r, col = "dodgerblue")
					plot(tmp3, col = c("transparent", "red"))
					plot(locked_in, add = T, col = c("transparent", "pink"))
					plot(locked_out, add = T, col = c("transparent", "pink"))
					plot(pa_sp, add = T, col = "black")
					...
					corr_overlap > 100000
						...
						#  Set up problem to add planning units to first planning unit selected.
						p_loop <- problem(x = pu_r, features = corr_sim_m) %>%
							add_max_utility_objective(budget = locked_in_area + 15001) %>%
							add_locked_in_constraints(locked_in) %>%
							add_locked_out_constraints(locked_out) %>%
							#add_contiguity_constraints() %>%
							#add_max_cover_objective(budget = locked_in_area + 50001) %>%
